...
class TextStyle:
    ...
    is_dark_theme: bool = False  # 是否为暗色主题
...
def add_title_image(background: Image.Image, title_image: str, rect_x: int, rect_y: int, rect_width: int) -> int:
    """添加标题图片
    
    Args:
        background: 背景图片
        title_image: 图片路径或URL
        rect_x: 矩形x坐标
        rect_y: 矩形y坐标
        rect_width: 矩形宽度
    """
    try:
        ...
        print(f"Error adding title image: {e}")
        return rect_y + 30  # 如果添加图片失败，返回一个默认的边距
