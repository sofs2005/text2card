...
def parse_color_tags(text):
    ...
    strikethrough_pattern = r'<span\s+style=["\']strikethrough["\']>(.*?)</span>'
    code_pattern = r'<span\s+style=["\']code["\']>(.*?)</span>'
    
    # 合并所有模式
    all_patterns = [
        (color_pattern, lambda m: TextSegment(m.group(3), TextStyle(color=m.group(1)))),
        (bold_pattern, lambda m: TextSegment(m.group(1), TextStyle(is_bold=True))),
        (italic_pattern, lambda m: TextSegment(m.group(1), TextStyle(is_italic=True))),
        (strikethrough_pattern, lambda m: TextSegment(m.group(1), TextStyle())),  # 暂不处理删除线
        (code_pattern, lambda m: TextSegment(m.group(1), TextStyle())),  # 暂不处理代码样式
    ]
    
    # 如果文本中没有任何标签，直接返回一个默认段落
    if not any(re.search(pattern, text) for pattern, _ in all_patterns):
    ...
    while current_pos < len(text):
        ...
        # 查找最早出现的标签
        for idx, (pattern, _) in enumerate(all_patterns):
            match = re.search(pattern, text[current_pos:])
            if match:
                match_start = current_pos + match.start()
                if earliest_match is None or match_start < earliest_match[0]:
                    earliest_match = (match_start, match)
                    earliest_pattern_idx = idx
        
        if earliest_match:
            match_start, match = earliest_match
            pattern, segment_creator = all_patterns[earliest_pattern_idx]
            ...
            # 处理标签
            segments.append(segment_creator(match))
            
            # 更新位置
            current_pos = match_start + len(match.group(0))
            found_match = True
    ...
    return segments if segments else [TextSegment(text, TextStyle(color='black'))]
